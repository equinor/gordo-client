...
def test_write_to_influx_with_retries(mock_influx_dataframe_client, caplog):
    ...
    with patch.object(
    ...
    ) as _stack_to_name_value_columns, patch("time.sleep"), caplog.at_level(logging.INFO):
        ...
        find_caplog_record("ERROR", "Failed to forward data to influx. Error: Connection refused")
...
def test_influx_forwarder(influxdb, influxdb_uri, sensors, sensors_str):
    ...
    assert all(c in name1_results.columns for c in ["machine", "sensor_name", "sensor_value"])
    ...
    for i, tag in enumerate(sensors_str):
        ...
        assert np.allclose(df[("name1", i)].values, name1_results[results_mask]["sensor_value"].values)
    ...
    assert all([c in name2_results.columns for c in ["machine", "sensor_name", "sensor_value"]])
    ...
    for key in output_keys:
        ...
        assert np.allclose(df[key].values, name2_results[results_mask]["sensor_value"].values)
...
def test_influx_send_data(influxdb, influxdb_uri, sensors, sensors_str):
    ...
    for key in sensors_str:
        ...
        assert np.allclose(df[key].values, resampled_results[results_mask]["sensor_value"].values)
