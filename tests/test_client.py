...
from gordo.client import utils as client_utils
...
def test_client_predictions_diff_batch_sizes(
    ...
    data_provider = providers.InfluxDataProvider(
        ...
        value_name="Value",
        client=client_utils.influx_client_from_uri(uri=influxdb_uri, dataframe_client=True),
    )

    prediction_client = Client(
        project=gordo_project,
        data_provider=data_provider,
        prediction_forwarder=ForwardPredictionsIntoInflux(destination_influx_uri=influxdb_uri),  # type: ignore
    ...
    vals = test_client.query(query)
    assert len(vals) > 0, f"Expected new values in 'predictions' measurement, but found {vals}"
...
def test_client_cli_basic(args):
    ...
    assert out.exit_code == 0, f"Expected output code 0 got '{out.exit_code}', {out.output}"
...
def test_client_cli_download_model(gordo_project, gordo_single_target, ml_server, tmpdir):
    """
    Test proper execution of client predict sub-command
    """
    runner = CliRunner()
...
def test_client_cli_predict(
    ...
    influx_client = client_utils.influx_client_from_uri(uri=influxdb_uri, dataframe_client=True)
    query = """
        SELECT *
        FROM "resampled"
        """
    ...
    args.extend(["--data-provider", json.dumps(providers.RandomDataProvider().to_dict())])
...
def test_client_cli_predict_non_zero_exit(
    ...
    data_provider = providers.InfluxDataProvider(measurement=influxdb_measurement, value_name="Value", uri=influxdb_uri)
